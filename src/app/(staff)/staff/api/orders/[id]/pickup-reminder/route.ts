  ...
  try {
    ...
    if (messages.length > 0) {
      try {
        ...
        console.error("pickup-reminder: order activity write failed", logErr);
      }
    }
    ...
    console.error("pickup-reminder route crashed:", err);
    return NextResponse.json(
      { error: err?.message || "Internal error sending pickup reminder" },
      { status: 500 }
    );
  }
}
