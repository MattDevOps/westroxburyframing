...
export async function POST(req: Request, ctx: Ctx) {
  try {
    ...
    try {
      ...
      console.error("notify-pickup-email: activity log write failed", logErr);
    }
    ...
    console.error("notify-pickup-email route crashed:", err);
    return NextResponse.json(
      { error: err?.message || "Internal error sending pickup email" },
      { status: 500 }
    );
  }
}
