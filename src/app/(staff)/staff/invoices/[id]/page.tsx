...
export default function InvoiceDetailPage({
  ...
  // Bill To state (company being billed, with the customer as the ATTN line)
  ...
  async function saveBillTo() {
    ...
    try {
      const res = await fetch(`/staff/api/invoices/${id}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ billToCompany: billToDraft.trim() }),
      });
      const data = await res.json();
      if (!res.ok) {
        alert(data.error || "Failed to update bill to");
        return;
      }
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-10 space-y-6">
      ...
      {/* Bill To — the company on the invoice, with the customer as the ATTN line */}
      {invoice.customer && (
        <div className="rounded-2xl border border-neutral-200 bg-white p-5">
          <div className="flex items-start justify-between gap-4">
            <div>
              <h3 className="font-semibold text-neutral-900 mb-2">Bill To</h3>
              {editingBillTo ? (
                <div className="space-y-2">
                  <input
                    value={billToDraft}
                    onChange={(e) => setBillToDraft(e.target.value)}
                    placeholder="Company name — blank bills the customer directly"
                    className="w-full sm:w-80 rounded-xl border border-neutral-300 px-3 py-2 text-sm"
                  />
                  <div className="flex gap-2">
                    <button
                      onClick={saveBillTo}
                      disabled={savingBillTo}
                      className="rounded-xl bg-black text-white px-3 py-1.5 text-xs disabled:opacity-50"
                    >
                      {savingBillTo ? "Saving…" : "Save"}
                    </button>
                    <button
                      onClick={() => setEditingBillTo(false)}
                      className="rounded-xl border border-neutral-300 px-3 py-1.5 text-xs text-neutral-700 hover:bg-neutral-50"
                    >
                      Cancel
                    </button>
                  </div>
                ...
                <div className="text-sm text-neutral-700">
                  {invoice.billToCompany ? (
                    <>
                      <div className="font-medium">{invoice.billToCompany}</div>
                      <div>
                        ATTN: {invoice.customer.firstName} {invoice.customer.lastName}
                      </div>
                    </>
                  ) : (
            ...
            {!editingBillTo && (
              <button
                onClick={() => {
                  setBillToDraft(invoice.billToCompany || "");
