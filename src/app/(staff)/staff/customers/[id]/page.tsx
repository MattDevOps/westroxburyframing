...
export default function CustomerDetailPage({ params }: any) {
  ...
  async function load() {
    ...
    try {
      const res = await fetch(`/staff/api/customers/${params.id}`, { cache: "no-store" });
  ...
  async function save(e: any) {
    ...
    try {
      const res = await fetch(`/staff/api/customers/${params.id}`, {
        method: "PATCH",
    ...
    <div className="p-6 space-y-6">
      <div className="flex items-start justify-between gap-4">
        ...
        <div className="flex gap-2">
          <a href="/staff/customers" className="rounded-xl border border-neutral-300 px-4 py-2 text-sm">
            Back
          </a>
          <a
      ...
      {msg ? (
        <div className="rounded-xl border border-green-300 bg-green-50 p-3 text-sm text-green-800">{msg}</div>
      ) : null}
          ...
          <form onSubmit={save} className="space-y-3">
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <input name="first_name" defaultValue={customer.firstName || ""} className="w-full rounded-xl border border-neutral-300 bg-white/5 px-3 py-2 text-sm" />
              </div>
              <div>
                <label className="block text-xs text-neutral-600 mb-1">Last name</label>
                <input name="last_name" defaultValue={customer.lastName || ""} className="w-full rounded-xl border border-neutral-300 bg-white/5 px-3 py-2 text-sm" />
              </div>
            </div>

            <div>
              <label className="block text-xs text-neutral-600 mb-1">Phone</label>
              <input name="phone" defaultValue={customer.phone || ""} className="w-full rounded-xl border border-neutral-300 bg-white/5 px-3 py-2 text-sm" />
            </div>

            <div>
              <label className="block text-xs text-neutral-600 mb-1">Email</label>
              <input name="email" defaultValue={customer.email || ""} className="w-full rounded-xl border border-neutral-300 bg-white/5 px-3 py-2 text-sm" />
            </div>

            <div className="grid grid-cols-2 gap-3 items-end">
              <div>
                <label className="block text-xs text-neutral-600 mb-1">Preferred contact</label>
                <select
              ...
              <label className="flex items-center gap-2 text-sm text-neutral-700">
                <input type="checkbox" name="marketing_opt_in" defaultChecked={Boolean(customer.marketingOptIn)} />
                Marketing opt-in
              </label>
            </div>

            <button disabled={saving} className="rounded-xl bg-black text-white px-4 py-2 text-sm">
              {saving ? "Saving…" : "Save"}
            </button>
          </form>
        </div>
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <div className="text-sm font-medium">
                        {o.orderNumber ? `#${o.orderNumber}` : "Order"} · {o.status || ""}
                      </div>
