...
export async function POST(request: Request) {
    try {
        ...
        if (event === "invitee.created") {
            ...
            // Use field names matching the Prisma Appointment model
            await prisma.appointment.create({
                data: {
                    customerId,
                    provider: "calendly",
                    externalEventId: calendlyUri,
                    name: name || "Guest",
                    email: email || "",
                    phone: phone || null,
        ...
        if (event === "invitee.canceled") {
            ...
            if (calendlyUri) {
                const existing = await prisma.appointment.findFirst({
                    where: { externalEventId: calendlyUri },
                });
                if (existing) {
                    await prisma.appointment.delete({
                        where: { id: existing.id },
                    });
                    console.log(`[Calendly] Appointment cancelled/deleted: ${calendlyUri}`);
                }
            }
            return NextResponse.json({ ok: true });
        }
